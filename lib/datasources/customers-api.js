...
module.exports = class CustomersApi extends RESTDataSource {
  ...
  async deployToLocation(client, { name, location, version, timestamp }) {
   const success = await client.hmset(`locations:${name}`,
    location, JSON.stringify( { version, timestamp })
   );
   return {
     success: success === 'OK',
  ...
  async updateMasterVersion(client, { name, version }) {
    const pattern = `*:customer:${name}`;
    const keys = await DataUtil.scan(client, 0, [], pattern);
    const key = keys[0];
    const success = await client.hset(key, 'masterVersion', version);
   return {
     success: success === 'OK',
     message: `successfully updated master version of ${name} to ${version}`
   }
  }
